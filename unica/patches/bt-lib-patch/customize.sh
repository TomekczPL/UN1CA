if [ ! -f "$WORK_DIR/system/system/lib64/libbluetooth_jni.so" ]; then
    ...
    # apex_payload.img is an ext4 image, debugfs can extract from it without root
    if command -v debugfs &> /dev/null; then
        ...
        [ -s "$WORK_DIR/system/system/lib64/libbluetooth_jni.so" ] \
            || rm -f "$WORK_DIR/system/system/lib64/libbluetooth_jni.so"
    fi
